...
comps <- lapply(setdiff(unique(dt$method), 'PCA'), function(x) c('PCA', x))
...
Fig <- plot_grid(plot_grid(p1, p2, labels = c('A', 'B'), ncol = 1),
...
top_vars <- get_top_by_label(dt[padj < 0.05][order(padj)], 'ref_cl', 10)
s <- unique(top_vars$variable)
pheatmap(M[,s], show_rownames = F)
plot_tsne(M[,s], ct)
plot_umap(M, ct)
plot_tsne(M, ct)
clust <- get_spectral_clusters(M, k_vals = 2:6)
clin <- process_clinical_covariates(rownames(M))
prog_vars <- get_prognostic_covariates(surv, clin, M)
prog.clust <- get_kmeans_subtypes(M[,unique(prog_vars$variable)], k_vals = 2:8, min_cluster_size = 0)

which.min(sapply(colnames(prog.clust), function(x) {
  plot_survival(surv, prog.clust[,x,drop=F], return_pval = T)
}))

plot_survival(surv, prog.clust[,'kmeans_3', drop = F])
plot_survival(surv, data.frame('ct' = ct, row.names = rownames(M)), 
              'PFI.time', 'PFI')

df <- plot_tsne(M[,unique(prog_vars$variable)], ct, returnData = T)
df$pfi_subtype <- as.factor(prog.clust[rownames(df),'kmeans_3'])

ggplot(df, aes(x = tSNE1, y = tSNE2)) + 
  geom_point(aes(color = as.factor(pfi_subtype))) +
  facet_wrap(~ group, nrow = 1 )

ggplot(df, aes(x = tSNE1, y = tSNE2)) + 
  geom_point(aes(color = group)) +
  facet_wrap(~ as.factor(pfi_subtype), nrow = 1 )

scores <- sapply(simplify = F, c('msigdb_hallmarks', 'msigdb_canonical_pathways', 'cancersea'), 
       function(x) {
         scores <- import_geneset_scores(geneSetScoresFolder, x)
         S <- scores[intersect(rownames(M), rownames(scores)),]
         return(S)
       })

pathways.prog <- get_factor_specific_variables(scores$cancersea, df$pfi_subtype)[padj < 0.05][order(padj)]
pathways.ct <- get_factor_specific_variables(scores$cancersea, df$group)[padj < 0.05][order(padj)]


get_top_by_label(pathways.ct[padj < 0.05][order(padj)], 'ref_cl', 10)
