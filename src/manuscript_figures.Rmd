...
dt$variable <- ifelse(dt$variable == 'base', 'Age+gender+stage', 'Age+gender+stage + MAUI LFs')
...
Legends[[fig_count]] <- "A) Top significant (padj < 0.05) clinical factor-associated multi-omic fingerprints per cancer type. B) Harrell’s C-index for progression-free survival prediction accuracy computed using only clinical factors (age + gender + tumor stage) or using clinical factors along with survival-predictive MAUI fingerprints"
