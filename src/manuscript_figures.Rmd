...
Figures <- c() # accummulate all figures in this
SuppFigures <- c() # accumulate all supplementary figures in this
...
Figures <- c(Figures, Fig)
ggsave('pancancer_cancer_type_prediction.pdf', plot = Fig, units = 'in', width = 12, height = 9)
...
Fig <- plot_grid(plot_grid(p1, p2, nrow = 2), p3, ncol = 2, rel_widths = c(2, 3), 
          labels = 'AUTO')
print(Fig)

ggsave('pancancer_subtype_prediction.pdf', plot = Fig, units = 'in', width = 12, height = 9)

SuppFigures <- c(SuppFigures, Fig)
```
...
ggsave('cluster_stability_metrics.pdf', plot = Fig, units = 'in', 
       width = 10, height = 8)
...
surv.stats <- pbapply::pbsapply(cl = cl, simplify = F, experiments, function(experiment) {
    ...
    res <- get_prognostic_covariates(surv, clin, M)
    if(!is.null(res)) {
      cancer_type <- strsplit(experiment, "\\.")[[1]][1]
...
ggsave(filename = 'surv.Cindex_improv.hazard_ratio.pdf', plot = p, width = 12, height = 14, units = 'in')
...
Fig <- cowplot::plot_grid(p1, p3, p2.1, p2.2, labels = 'AUTO')
ggsave(filename = 'MSI.status.pdf', Fig, units = 'in', width = 10, height = 8)
...
# TODO: update this to point to /data/local/buyar/arcas/subtyping_paper/data/geneset_scores/singscore
# and remove paths from script (put it in settings.yaml)
...
# TODO: update this to point to /data/local/buyar/arcas/subtyping_paper/data/geneset_scores/singscore
# and don't use hard-coded paths 
