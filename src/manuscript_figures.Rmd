...
p2 <- ggplot(stats[purity_method == 'CPE'][grep('TCGA', project)], 
       aes(x = reorder(project, COR), y = COR, fill = COR)) + 
  geom_bar(stat = 'identity') +
  coord_flip() +
  theme(axis.title.y = element_blank(),  legend.position = 'none') + 
...
selected <- stats[purity_method == 'IHC'][grep('TCGA', experiment)][order(COR, decreasing = T)][1:4]$experiment
...
p3 <- cowplot::plot_grid(plotlist = plots, ncol = 1)

# supplementary figure
supFig <- cowplot::plot_grid(cowplot::plot_grid(p1, p2, labels = c('A', 'B')), 
...
B) Correlation of the MAUI predictions of CPE versus the annotated CPE levels (CPE stands for consensus purity estimated by computational methods: ABSOLUTE, ESTIMATE, and LUMP). 
C) Scatterplots of MAUI predictions versus the annotated IHC and CPE values for the 
top best predicted cancer types."
```
