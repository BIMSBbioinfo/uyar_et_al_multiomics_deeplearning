...
params:
  settings_file: 'settings.yaml' # settings file for pancancer analysis
  immther_projdir: 'immunotherapy_analysis' # settings file for immunotherapy analysis 
  tmz_gbm_figure_data_folder: 'data/tmz_gbm_figure_data' # path to data folder for tmz-gbm analysis 
...
knitr::opts_chunk$set(echo = FALSE, include = FALSE, warning = FALSE, message = FALSE, 
                      fig.width = 9, fig.height = 11, eval = TRUE)
...
hm <- apply(m, 1, function(x) round(x/sum(x) * 100))
hm_labels <- hm
hm_labels[hm_labels < 2] <- '' #don't display very low values
p2 <- pheatmap::pheatmap(hm, cluster_rows = F, cluster_cols = F, 
          silent = T, cellwidth = 10, cellheight = 10, color = colorRampPalette(c('white', 'red'))(10), 
          display_numbers = hm_labels, fontsize = 8)
...
dt <- get_factor_specific_variables(M, ct)[padj < 0.05][order(padj)]
top_vars <- get_top_by_label(dt, 'ref_cl', topN = 5)

p3 <- plot_tsne(M[,unique(top_vars$variable)], ct, show.labels = T) + 
...
p5 <- p5 + labs(y = 'Relative Contribution\nto top latent factor') + 
  theme(legend.position = 'right', legend.direction = 'vertical', 
        ...
        legend.title = element_blank(), legend.box.just = 'left')
...
Legends[[fig_count]] <- " A) tSNE plot of MAUI multi-omics fingerprints (using top 5 fingerprint enriched per cancer type) learned from pan-cancer dataset  B) Percentage of top 100 features by omics types contributing to top predictive MAUI fingerprint per cancer type. C, D) Comparison of unsupervised clustering performances based on cluster homogeneity (Silhouette) in C and cluster stability (Average Proportion of Non-Overlap (APN) in D. Each point represents a score for an individual clustering experiment in a total of 132 k-means clustering experiments (21 cancer types x for 6 different values of k from 3 to 8) for each tool."
...
level of classification accuracy. C) Top MAUI latent factors (multi-omic fingerprints) detected per cancer type. Size/color/opacity of the points represent the average neural activation value of the MAUI fingerprint for the corresponding type of tumor samples."
```
...
# make example plots for selected clinical features
pr <- 'TCGA-LGG' 
datatype <- 'cnv_gex_meth_mut'
M <- LFs$maui_factors[[paste0(pr, '.', datatype)]]
clin <- process_clinical_covariates(rownames(M))
dt.pr <- dt[padj < 0.05][project == pr][order(padj)]
# select top 3 clinical features for the project
plots <- lapply(head(dt.pr$label, 3), function(x) {
  # get lf, clinical feature and plot distribution 
  f <- gsub("[\n ]", "_", x)
  lf <- dt.pr[label == x]$variable
  df <- cbind(data.frame(M[,lf,drop = F]), clin[,f,drop=F])
  df <- df[!is.na(df[,f]),]
  ggpubr::ggboxplot(df, y = lf, x = f, 
                    color = f, add = 'jitter') + 
    labs(x = x) + 
    theme(legend.position = 'none', axis.text.x = element_text(angle = 45, hjust = 1)) 
})

p2 <- cowplot::plot_grid(plotlist = plots, nrow = 1, align = 'h')
...
dt$variable <- ifelse(dt$variable == 'base', 'Age+gender+stage', 'Age+gender+stage + MAUI fingerprints')

p3 <- ggplot(ci, aes(x = reorder(project, with_lf))) + 
  geom_segment(aes(x = reorder(project, with_lf), xend = project, 
                   y = base, yend = with_lf)) + 
  geom_hline(yintercept = c(0.5, 0.6, 0.7), linetype = 2) + 
...
Legends[[fig_count]] <- "A) Top significant (padj < 0.05) clinical factor-associated multi-omic fingerprints per cancer type. B) MAUI fingerprints enriched for subgroups of different clinical features of TCGA-LGG samples. 
LF45 is enriched among samples histologically categorized as Oligodendrogliomas; LF54 is enriched among donors of male gender; LF65 is enriched among samples with histological grade of G3. C) Harrell’s C-index for progression-free survival prediction accuracy computed using only clinical factors (age + gender + tumor stage) or using clinical factors along with survival-predictive MAUI fingerprints"
...
SuppLegends[[supFig_count]] <- "Kaplan-Meier survival curves adjusted for clinical factors (tumor stage + age + gender) stratified by the neural activation values of top survival-predictive multi-omic fingerprints per cancer."
```

```{r tumor_purity} 
experiments <- grep('TCGA.*cnv_gex_meth_mut', names(LFs$maui_factors), value = T)
# use maui LFs to predict TCGA IHC/CPE values that represent tumor purity levels 
cl <- parallel::makeCluster(20)
parallel::clusterExport(cl, varlist = c('LFs', 'experiments', 'settings'))
results <- pbapply::pbsapply(cl = cl, simplify = F, experiments, function(experiment) {
    message(experiment)
    source(settings$utility_script)
    library(data.table)
    M <- LFs$maui_factors[[experiment]]
    tp <- get_tumor_purity_estimates(rownames(M))
    if(nrow(tp) < 50) {
      warning("Couldn't find at least 100 samples with tumor purity 
              scores in experiment ", experiment, " => ", nrow(tp))
      return(NULL)
    }
    # predict IHC
    y1 <- tp[,mean(IHC), by = bcr_patient_barcode][match(rownames(M), bcr_patient_barcode)]$V1
    res1 <- predict_numerical_glmnet(M, y1)
    # predict CPE (consensus estimates)
    y2 <- tp[,mean(CPE), by = bcr_patient_barcode][match(rownames(M), bcr_patient_barcode)]$V1
    res2 <- predict_numerical_glmnet(M, y2)
    return(list('IHC' = res1, 'CPE' = res2))
})
parallel::stopCluster(cl)

# summarize stats 
stats <- do.call(rbind, lapply(names(results), function(x) {
  message(x)
  if(is.null(results[[x]])) {
    return(NULL)
  }
  dt <- do.call(rbind, lapply(names(results[[x]]), function(y) {
    message(y)
    dt <- evaluate_regression_model(results[[x]][[y]][['preds']]$y, 
                              results[[x]][[y]][['preds']]$y_hat)
    dt$purity_method <- y
    return(dt)
  }))
  dt$experiment <- x
...
stats$project <- unlist(lapply(strsplit(stats$experiment, "\\."), function(x) x[[1]]))


# plot correlation scores for all cohorts 
p1 <- ggplot(stats[purity_method == 'IHC'][grep('TCGA', project)], 
       aes(x = reorder(project, COR), y = COR, fill = COR),) + 
  geom_bar(stat = 'identity') +
  coord_flip() +
  theme(axis.title.y = element_blank(), legend.position = 'none') + 
  labs(y = "MAUI vs IHC") + ylim(0, 1)

# compute other tools' correlations with IHC
tp <- get_tumor_purity_estimates()
absolute <- tp[!is.na(ABSOLUTE), mean(cor(ABSOLUTE, IHC, use = 'complete.obs')),by = project]
estimate <- tp[!is.na(ESTIMATE), mean(cor(ESTIMATE, IHC, use = 'complete.obs')),by = project]
lump <- tp[!is.na(LUMP), mean(cor(LUMP, IHC, use = 'complete.obs')),by = project]

dt <- stats[purity_method == 'IHC']
colnames(dt)[3] <- 'MAUI'
dt$ABSOLUTE <- absolute[match(dt$project, project)]$V1
dt$LUMP <- lump[match(dt$project, project)]$V1
dt$ESTIMATE <- estimate[match(dt$project, project)]$V1

mdt <- melt(dt, measure.vars = c('MAUI', 'ABSOLUTE', 'ESTIMATE', 'LUMP'), id.vars = 'project')
p2 <- ggboxplot(mdt, x = 'variable', y = 'value', add = 'jitter', color = 'variable') +
  theme(legend.position = 'none', axis.title.x = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1)) + 
  labs(y = 'Average Correlation with IHC')

p3 <- ggplot(stats[purity_method == 'CPE'][grep('TCGA', project)], 
       aes(x = reorder(project, COR), y = COR, fill = COR)) + 
  geom_bar(stat = 'identity') +
  coord_flip() +
  theme(axis.title.y = element_blank(),  legend.position = 'none') + 
  labs(y = "MAUI vs CPE") + ylim(0, 1)

# correlation plots for top cancer types
selected <- stats[purity_method == 'IHC'][grep('TCGA', experiment)][order(COR, decreasing = T)][1:2]$experiment
plots <- lapply(selected, function(x) {
  message(x)
  p <- cowplot::plot_grid(plotlist = lapply(names(results[[x]]), function(y) {
    message(y)
    ggpubr::ggscatter(results[[x]][[y]][['preds']], 
                           x = 'y_hat', y = 'y', 
                           add = 'reg.line', cor.coef = T, cor.method = 'pearson') + 
      labs(x = paste('MAUI prediction of', y), y = y, title = gsub("\\..+$", "", x))
  }))
  return(p)
})

p4 <- cowplot::plot_grid(plotlist = plots, ncol = 1)

# supplementary figure
supFig <- cowplot::plot_grid(cowplot::plot_grid(p1, p2, p3, labels = c('A', 'B', 'C'), nrow = 1), 
                             p4, nrow = 2, labels = c('', 'D'))

# print p3 to supplements 
supFig_count <- supFig_count + 1
SuppFigures[[supFig_count]] <- supFig
fig.name <- paste0('SuppFigure', supFig_count, '.tumor_purity.supp.pdf')
names(SuppFigures)[supFig_count] <- fig.name
ggsave(file.path(FigureFolder, fig.name), 
              plot = supFig, units = 'in', width = 8.27, height = 11.69)
SuppLegends[[supFig_count]] <- "Predicting tumor purity estimates using MAUI latent factors. 
A) Correlation of the MAUI predictions of IHC versus the annotated IHC values (immunohistochemistry-based tumor purity estimation). 
B) Comparison of average correlations between computational estimates of IHC values by different tools. Each point represents the 
value obtained for each of the TCGA cohorts seen in panel A. ABSOLUTE tool lacks estimates for 6 out of 14 cancer types. 
C) Correlation of the MAUI predictions of CPE versus the annotated CPE levels (CPE stands for consensus purity estimated by computational methods: ABSOLUTE, ESTIMATE, and LUMP). 
D) Scatterplots of MAUI predictions versus the annotated IHC and CPE values for the 
top two best predicted cancer types with respect to annotated IHC values."
```
...
ggsave(filename = file.path(FigureFolder, fig.name), 
       plot = Fig, units = 'in', width = 12, height = 12)
Legends[[fig_count]] <- "Latent factors predictive of MSI status (high: MSI-H, stable: MSI-S, low: MSI-L) in pan-gastrointestinal cancers A) Comparison of top latent factor per MSI status for each tool in terms of adjusted p-values reflecting the differences between MSI subgroups. B) ROC curves for prediction accuracy of MSI status using MAUI latent factors (multi-omic fingerprints) C) tSNE plots of samples generated using top MSI-predictive fingerprints (5 LF per MSI class) colored by MSI status D) tSNE plots of samples colored by MAUI fingerprint LF25, which is most predictive of MSI-H samples. E) Distribution of MAUI fingerprint LF25 across MSI subgroups in the pan-gastrointestinal cancer samples. F) Relative abundance of omics feature types among the top 500 features contributing to MAUI fingerprint LF25."
```
...
top_markers <- map_ids(names(markers), cpg2hgnc, ens2hgnc)
# get unique table 
top_markers <- top_markers[match(top_markers[,head(variable, 1), by = c('type', 'hgnc')]$V1, variable)]
...
# further filter top features: build initial tree to get subset of most important features
x = partykit::ctree(subtype ~ ., data = df.train)
selected <- names(partykit::varimp(x))
df.train <- df.train[,c(selected, 'subtype')]
df.test <- df.test[,c(selected, 'subtype')]
...
Legends[[fig_count]] <- "Classification of Non-Small-Cell Lung Adenocarcinoma samples using MAUI multi-omic fingerprints.
A) ROC curves with AUC values on testing data (40% of the dataset) using an Elastic Net model with 5-fold cross-validation. B) t-SNE plot of MAUI fingerprints colored by cancer types. C) t-SNE plot of MAUI fingerprints colored by 
the top most-predictive fingerprint for TCGA-LUSC samples. D) t-SNE plot of MAUI fingerprints colored by the top 
most predictive fingerprint for TCGA-LUAD samples. E) Decision tree visualisation of top MAUI fingerprint-associated omics markers detected that can be used to distinguish LUSC and LUAD subtypes."
```

```{r immunotherapy_response}
# immunotherapy response prediction analysis: 
projDir <- params$immther_projdir 
settings_imtrp <- yaml::read_yaml(file.path(projDir, 'settings_immther.yaml'))
gex_all <- readRDS(file.path(projDir, 'data', 'mariathasan.gex.all_genes.normalised.RDS'))
human_gene_signatures <- readRDS(file.path(projDir, 'data', 'human_gene_signatures.RDS'))

LFs_imtrp <- import_LFs(c('maui_factors', 'mofa_factors', 'pca_factors'), 
                        file.path(projDir, 'output'))
FWs_imtrp <- import_feature_weights(c('maui_factors'), file.path(projDir, 'output'))
colData <- data.table::fread(file.path(projDir, 'data', 'mariathasan.colData.csv'))
...
p1 <- ggplot(dt[,.SD[which.min(padj)],by = c('tool', 'ref_cl')], 
  ...
  theme(legend.title = element_blank(), axis.title.y = element_blank(), 
        plot.title = element_text(hjust = 0.5), 
        legend.key.size = unit(0.2, units = 'in'))
...
dt$tool <- toupper(gsub("_factors", "", dt$tool))
p2 <- ggplot(dt, aes(x = tool, y = abs(V1))) + 
  geom_bar(stat = 'identity', aes(fill = tool), position = 'dodge') +
  coord_flip() + 
...
dt$tool <- toupper(gsub("_factors", "", dt$tool))
p3 <- ggplot(dt[,.SD[which.min(padj)],by = c('ref_cl', 'tool')], 
             aes(x = ref_cl, y = -log10(padj))) + 
  geom_bar(stat = 'identity', aes(fill = tool), position = 'dodge') +
  geom_hline(yintercept = -log10(0.05)) + 
...
# test comparison of maui LFs and published markers from Mariathasan et al
# in terms of response prediction
# published markers from Mariathasan et al (https://www.nature.com/articles/nature25501#Sec4)
# 1. TMB (tumor neoantigen burden) 2. CD8 effector T cell markers (8 genes)
# 3. F-TBRS: fibroblast tgf-beta response signature (4 genes see https://www.nature.com/articles/nature25501/figures/3)

# find top maui LFs informative for response prediction 
M <- scale(LFs_imtrp$maui_factors$mariathasan.gex)
labels <- colData[match(rownames(M), colData$bcr_patient_barcode),]$binaryResponse
df <- cbind(data.frame(M), 'y' = labels)
df <- df[!is.na(df$y),]
df$y <- as.factor(df$y)
# build an initial glmnet model using all LFs on training data
fit <- predict_categorical(df, 'CR/PR') 
# get subset of top LFs 
v <- caret::varImp(fit)[['importance']]
top_lfs <- data.table(v, keep.rownames = T)[order(Overall, decreasing = T)][Overall > 0]$rn

# make a heatmap of top predictive lfs 
m <- as.matrix(df[,-ncol(df)])
f <- ifelse(df$y == 'CR/PR', 'Responders', 'Non-responders')
p4 <- pheatmap::pheatmap(get_basis_matrix(m[,top_lfs], f), silent = T, cellheight = 50, cluster_rows = F)
...
# get the assay data for the studied markers for CD8 effectors + TBRS etc.
signatures <- list('cd8_Teff' = human_gene_signatures$`CD 8 T effector`, 
                   'F_TBRS' = c('ACTA2', 'COL4A1', 'TAGLN', 'SH3PXD2A'))
# make a data frame of marker genes + TMB measures + top MAUI LFs
DF <- merge(data.frame(t(gex_all[unlist(signatures),])), 
            data.frame(M[,top_lfs]), by = 'row.names')
DF$TMB <- colData[match(DF$Row.names, colData$bcr_patient_barcode),]$`Neoantigen burden per MB`
# response variable
DF$y <- colData[match(DF$Row.names, colData$bcr_patient_barcode),]$binaryResponse
# remove samples without response variable
DF <- DF[!is.na(DF$y),]
# remove samples without TMB values 
DF <- DF[!is.na(DF$TMB),]
DF <- data.frame(DF[,-1], row.names = DF$Row.names)
DF$y <- as.factor(DF$y)

L <- list('TMB' = 'TMB',
          'MAUI' = top_lfs, 
          'CD8_Teff + F_TBRS' = c(signatures$cd8_Teff, signatures$F_TBRS))
# now predict response using subsets of features 
# get combination of models
combs <- c(asplit(combn(names(L), 2), 2),
           asplit(combn(names(L), 3), 2), 
           'MAUI',
           'CD8_Teff + F_TBRS')
results <- do.call(rbind, lapply(combs, function(x) {
  comb <- paste0(x, collapse = " + ")
  message("trying combination:",comb)
  included <- as.vector(unlist(L[x]))  # variables included in the model
  message("including variables:",paste(included, collapse = ' '))
  res <- predict_cluster_labels_glmnet(scale(as.matrix(DF[,intersect(colnames(DF), included),drop = F])), 
                                DF$y, repeats = 5, sampling = 'down')
  res$comb <- comb
  return(res)
}))

results$comb <- stringr::str_wrap(results$comb, width = 12)
p6 <- ggplot(results[order(auc)][label == 'CR/PR'], 
       aes(x = reorder(comb, -auc), y = auc)) + 
  geom_bar(stat = 'identity', aes(fill = auc)) + 
  geom_errorbar(aes(ymin = auc.lower, ymax = auc.upper), stat = 'identity', 
                width = 0.1) + 
  labs(x = 'Marker Combination', y = 'AUC') + 
  theme(legend.position = 'none', axis.text.x = element_text(angle = 30, hjust = 1)) 

p5 <- p5$plot + theme(legend.title = element_blank(), legend.direction = 'vertical')
Fig <- cowplot::plot_grid(
  cowplot::plot_grid(p1, p3, p2, nrow = 1, labels = c('A', 'B', 'C')), 
  cowplot::plot_grid(p5, p4$gtable, nrow = 1, labels = c('D', 'E')), 
  cowplot::plot_grid(cowplot::ggdraw(), p6, cowplot::ggdraw(), rel_widths = c(1, 5, 1), nrow = 1, labels = c('', 'F', '')), 
  nrow = 3
)
fig_count <- fig_count + 1
Figures[[fig_count]] <- Fig
...
Cancers (N=216) using MAUI. A) Comparison of top latent factors predictive of complete/partial response (CR/PR) to the anti-PD-L1. B) Comparison of top latent factors predictive of immune phenotypes (inflamed/excluded/desert). C)  Comparison of top latent factors with respect to their correlation with tumor mutation burden status of the samples in the cohort D) Survival stratification of the cohort based activation score of top MAUI latent factor (fingerprint) E) Average value of top MAUI latent factors (fingerprints) predictive of response to anti-PD-L1 response groups.
F) Comparison of marker combinations in terms of accuracy in predicting anti-PD-L1 response. TMB: tumor mutation burden, 
CD8_Teff: 12 gene signature for CD8+ effector T-cells, F_TBRS: 4-gene pan-fibroblast TGF-beta response signature, MAUI: top predictive MAUI fingerprints as seen in panel E). Each bar plot represents prediction accuracy (AUC) when using the corresponding subset of variables in an Elastic Net model."
```
...
folder <- params$tmz_gbm_figure_data_folder
...
p1 <- ggpubr::ggbarplot(aucs, x = "method", y = 'AUC', add = 'mean_se',
                  fill = 'group') +
  coord_flip() + 
  theme(axis.title.y = element_blank(), legend.position = 'none') + 
...
# umap responders
umap <- data.table::fread(file.path(folder, 'umap_coords_and_response.csv'))
colnames(umap)[2:3] <- c('UMAP1', 'UMAP2')
#drop samples without umap coords
umap <- umap[!is.na(UMAP1)][!is.na(UMAP2)]
umap$response <- ifelse(umap$response == TRUE, 'responder', 'non-responder')
umap[is.na(response)]$response <- 'unavailable'
umap[is.na(response)]$color <- 'gray'
p2 <- ggscatter(umap, x = 'UMAP1', y = 'UMAP2', color = 'response', size = 2)  + 
  scale_color_manual(values = c('dodgerblue', 'firebrick', 'gray')) + 
  theme(legend.title = element_blank())

# survival plots
gbm_surv <- read.csv(file.path(folder, "survival_time_response_and_predictions.csv"), row.names = 1)
gbm_surv <- gbm_surv[,c(3, 5:7)]
panel_titles <- c('observed response', 'MAUI + MGMT', 
                  'MGMT-expression', 'MGMT-methylation')
plots <- sapply(simplify = F, 1:ncol(gbm_surv), function(i) {
  x <- colnames(gbm_surv)[i]
  plot_survival(surv, partition.df = gbm_surv[,x,drop = F], duration_col = 'PFI.time', event_col = 'PFI')[['plot']] + 
    ggtitle(label = panel_titles[i]) + theme(legend.direction = 'vertical', legend.title = element_blank(), 
                                             legend.position = c(0.5, 0.8))
})

Fig <- cowplot::plot_grid(cowplot::plot_grid(p1, p2, labels = c('A', 'B')),
                          cowplot::plot_grid(plotlist = plots, ncol = 2, labels = c('C', 'D', 'E', 'F')), 
...
names(Figures)[fig_count] <- fig.name
ggsave(file.path(FigureFolder, fig.name),
              plot = Fig, units = 'in', width = 9, height = 11)
Legends[[fig_count]] <- "Predicting TMZ resistance in GBM patients. A) Area under the ROC curve (AUC) for 5-fold cross-validation predictions of response to TMZ. MAUI multi-omic fingerprints outperform both MGMT expression and MGMT promoter methylation, and the best performance is achieved by combining MAUI fingerprints with MGMT expression. B) 2D UMAP projection of TCGA-GBM samples based on MAUI multi-omic fingerprints which are predictive of TMZ response. Responders and non-responders tend to cluster separately. C) Kaplan-Meier curves for progression-free interval for responders and non-responders to TMZ. Response defined as progression-free interval of at least six months, non- or partial-response defined as tumor progression within six months of treatment. D) Kaplan-Meier curves for progression-free intervals for predicted responders and non-responders using MAUI multi-omic fingerprints combined with MGMT expression. E) Kaplan-Meier curves for progression-free intervals for predicted responders and non-responders using MGMT expression. F) Kaplan-Meier curves for progression-free intervals for predicted responders and non-responders using MGMT promoter methylation."
