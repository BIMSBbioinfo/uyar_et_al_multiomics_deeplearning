...
cluster.stats <- do.call(rbind, sapply(simplify = F, names(LFs), function(tool) {
  ...
  stats <- do.call(rbind, pbapply::pbsapply(cl = cl, simplify = F, 
    ...
    stats <- do.call(rbind, lapply(seq(5, min(25, ncol(M)), 5), function(n) {
      message(n)
...
cl <- parallel::makeCluster(20)
parallel::clusterExport(cl, varlist = c('LFs', 'experiments', 'settings', 'surv'))
surv.stats <- do.call(rbind, sapply(simplify = F, names(LFs), function(tool) {
  ...
  do.call(rbind, pbapply::pbsapply(cl = cl, simplify = F, experiments, function(experiment) {
    ...
    M <- LFs[[tool]][[experiment]]
    if(tool == 'maui_factors') {
      M <- remove_redundant_variables(M, 99)
    }
    dt <- data.table('cindex.pfi' = NA, 'cindex.os' = NA, 'experiment' = experiment, 'method' = tool)
    # compute pfi stats
    vars <- get_prognostic_covariates(surv, clin = NULL, explored_covariates = M,
                                      event_col = 'PFI', duration_col = 'PFI.time')
    if(!is.null(vars)) {
      v <- head(vars[order(pval)][pval < 0.05][['variable']], 10)
      fit <- fit_RSF(surv, M[,v], 1000, 'PFI.time', 'PFI')
      dt$cindex.pfi <- round(1 - mean(fit$err.rate, na.rm = T), 2)
    }
    vars <- get_prognostic_covariates(surv, clin = NULL, explored_covariates = M,
                                      event_col = 'OS', duration_col = 'OS.time')
    if(!is.null(vars)) {
      v <- head(vars[order(pval)][pval < 0.05][['variable']], 10)
      fit <- fit_RSF(surv, M[,v], 1000, 'OS.time', 'OS')
      dt$cindex.os <- round(1 - mean(fit$err.rate, na.rm = T), 2)
    }
    return(dt)
  }))
}))
...
dt <- melt(surv.stats, measure.vars = c('cindex.pfi', 'cindex.os'))
dt[is.na(value)]$value <- min(dt$value, na.rm = T) # 
dt$method <- toupper(gsub("_factors", "", dt$method))
dt$project <- unlist(sapply(strsplit(dt$experiment, "\\."), function(x) x[1]))
dt$endpoint <- ifelse(dt$variable == 'cindex.pfi', 'Progression Free Survival', 
                      'Overall Survival')
# PCA/MOFA win, compare against PCA
comps <- lapply(setdiff(unique(dt$method), 'MAUI'), function(x) c('MAUI', x))
p4 <- ggboxplot(dt, x = 'method', y = 'value', color = 'method',
...
M <- remove_redundant_variables(LFs$maui_factors$nsclc.cnv_gex_meth_mut, perc = 99)
ct <- surv[match(rownames(M), bcr_patient_barcode)]$project
...
M <- remove_redundant_variables(LFs$maui_factors[[experiment]], perc = 99)
st <- tcga_subtypes[match(rownames(M), bcr_patient_barcode)]$Subtype_Selected
...
Fig1 <- cowplot::plot_grid(plotlist = bipartite_plots, nrow = 3, labels = 'AUTO')
ggsave('test1.pdf', plot = Fig1, units = 'in', width = 12, height = 14)

Fig <- cowplot::plot_grid(p1, p2, labels = 'AUTO', nrow = 2)
#ggsave('test2.pdf', plot = Fig2, units = 'in', width = 14, height = 12)
