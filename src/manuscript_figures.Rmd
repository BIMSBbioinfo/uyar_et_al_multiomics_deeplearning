...
res <- predict_cluster_labels_glmnet(M, ct)
...
supFig_count <- supFig_count + 1
SuppFigures[[supFig_count]] <- Fig
fig.name <- paste0('SuppFigure', fig_count, '.subtype_dependency.pdf')
names(Figures)[fig_count] <- fig.name
ggsave(file.path(FigureFolder, fig.name), 
              plot = Fig, units = 'in', width = 18, height = 12)
SuppLegends[[fig_count]] <- "Demonstration of how cancer subtypes are context/objective dependent.
