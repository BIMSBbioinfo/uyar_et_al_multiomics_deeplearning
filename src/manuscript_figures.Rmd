...
Legends[[fig_count]] <- "Predictive power of latent factors in distinguishing cancer types. A) comparing MAUI with PCA/MOFA/MCIA. Mean Balanced Accuracy computed on the test dataset (30% of held-out data) trained on 70% of the dataset using an Elastic Net multi-class model. B) tSNE plot of MAUI latent factors learned from pan-cancer dataset  C) Percentage of top feature types contributing to top predictive maui latent factor per cancer type. D) Top maui LFs detected per cancer type. Size/color/opacity of the points represent the average neural activation value of the LF for the corresponding cancer type samples."
...
ggsave(file.path(FigureFolder, fig.name),        
       plot = Fig, units = 'in', 
       width = 10, height = 10)
Legends[[fig_count]] <- "Comparison of unsupervised clustering performances based on cluster homogeneity (Silhouette) in A) and cluster stability (Average Proportion of Non-Overlap (APN) in B). Each point represents a clustering score for 132 k-means clustering results (21 cancer types x for 6 different values of k from 3 to 8) for each tool. C-D) Survival outcome prediction accuracy for 21 cancer types by each tool's top 10 survival-related factors based on Overall Survival (in C) or Progression Free Survival endpoints. (in D)."
```
...
Legends[[fig_count]] <- "Using MAUI LFs for progression-free survival prediction. A) Harrell’s C-index computed using only clinical factors (tumor stage + age + gender) or using clinical factors along with survival-predictive MAUI LFs. B) Hazard ratios of top LFs picked for each cancer adjusted for clinical factors (tumor stage + age + gender). C) Kaplan-Meier survival curves adjusted for clinical factors (tumor stage + age + gender) stratified by the neural activation values of top survival-predictive LFs per cancer."
```
...
Legends[[fig_count]] <- "A) Top significant (padj < 0.05) clinical factor-associated LFs per cancer type. B) Example case: top LFs associated with each gender groups of TCGA-THCA cohort. C) Example case: top LFs associated with each histological types of TCGA-UCEC."
```
...
ggsave(filename = file.path(FigureFolder, fig.name), 
       Fig, units = 'in', 
       width = 10, height = 12)
Legends[[fig_count]] <- "Clustering of pan-gastrointestinal tumors guided by TCGA subtypes using subtype-specific MAUI latent factors. A) tSNE plot of using MAUI latent factors for pan-gastrointestinal cancers colored by MAUI clusters, B) tSNE plot of using MAUI latent factors for pan-gastrointestinal cancers colored by known TCGA subtypes C) progression-free survival curves for MAUI-based clusters and D) known TCGA subtypes. E-F) Gene-set activity scores of annotated cancer hallmarks gene sets enriched for each MAUI-based cluster in E) and known TCGA subtypes in F)."
```
...
ggsave(filename = file.path(FigureFolder, fig.name), 
       plot = Fig, units = 'in', 
       width = 10, height = 12)
Legends[[fig_count]] <- "A) Log-rank test of prognostic subtypes by MAUI and annotated TCGA subtypes. B) tSNE plot of MAUI LFs that are predictive of survival outcomes for TCGA-PRAD. C) Kaplan-Meier curve of the best/worst surviving prognostic MAUI clusters D) Biological pathway scores that distinguish best/worst surviving clusters."
```
