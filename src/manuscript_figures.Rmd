...
# do unsupervised clustering experiment and compute AMI 
# downsample pan-cancer dataset 
df <- data.frame('id' = rownames(LFs$pca_factors$pancancer.cnv_gex_meth_mut))
df$pr <- surv[match(df$id, bcr_patient_barcode)]$project
ids <- lapply(split(df, df$pr), function(x) {
  if(nrow(x) > 200) {
    sample(x$id, 200)
  }
})
ids <- ids[lengths(ids) == 200]
k <- 5
# sample cancer types and compute ami for different tools 
sampled <- sapply(simplify = F, 1:50, function(i) {
  unique(unlist(ids[sample(1:length(ids), k)]))
})
cl <- parallel::makeCluster(10)
parallel::clusterExport(cl, varlist = c('ids', 'df', 'LFs', 'settings', 'sampled', 'k'))
res <- do.call(rbind, pbapply::pblapply(cl = cl, 1:length(sampled), function(i) {
  require(data.table)
  source(settings$utility_script)
  selected <- sampled[[i]]
  ct <- df[match(selected, df$id),]$pr
  message("selected cancers:",paste0(unique(ct), collapse = ','))
  do.call(rbind, lapply(names(LFs), function(tool) {
    M <- LFs[[tool]]$pancancer.cnv_gex_meth_mut[selected,]
    if(tool == 'maui_factors') {
      M <- scale(M)
    }
    ami <- compute_AMI(M, k, ct)
    return(data.table('i' = i, 'ami' = ami, 'method' = tool, 
                      'pr' = paste0(unique(ct), collapse = ',')))
  }))
}))
parallel::stopCluster(cl)
res$method <- toupper(gsub("_factors", "", res$method))


comps <- lapply(setdiff(unique(res$method), 'MAUI'), function(x) c('MAUI', x))
p5 <- ggboxplot(res, x = 'method', y = 'ami', add = 'jitter', color = 'method') + 
  stat_compare_means(method = 'wilcox.test', paired = TRUE, 
                     comparisons = comps) +
  labs(y = "Adjusted Mutual Information", x = '') +
...
fig.name <- paste0('SuppFigure', supFig_count, '.pancancer_subtype_prediction.pdf')
names(SuppFigures)[supFig_count] <- fig.name
ggsave(file.path(FigureFolder, fig.name), 
              plot = Fig, units = 'in', width = 18, height = 12)
SuppLegends[[supFig_count]] <- "Predictive power of latent factors in distinguishing known TCGA cancer subtypes A-B) comparing MAUI with PCA/MOFA/MCIA. AUC: Area under the ROC curve computed on the test dataset (30% of held-out data) trained on 70% of the dataset using a Random Forest model. Each point represents one cancer subtype (pairwise one-sided Wilcoxon test on AUCs, alternative = maui AUC is not greater than others) C) Top maui LFs detected per cancer subtype (only displaying subtypes with at least 20 samples). Size/color/opacity of the points represent the average neural activation value of the LF for the corresponding subtype samples. D) Percentage of top feature types contributing to top predictive maui latent factor per cancer subtype."
```
...
p1 <- ggboxplot(dt1, 
  ...
  stat_compare_means(method = 'wilcox.test', paired = FALSE, #todo change to true when mofa is finished
                     comparisons = comps) + 
...
# find survival-predictive LFs by univariate cox models and use them in RSF
...
top_vars <- unique(get_top_by_label(dt[padj < 0.05][order(padj)], 'ref_cl', 10)[['variable']])
...
p3.1 <- ggplot(df, aes(x = tSNE1, y = tSNE2)) + geom_point(aes(color = as.factor(group))) + 
  theme(legend.title = element_blank())
p3.2 <- ggplot(df, aes(x = tSNE1, y = tSNE2)) + 
...
p4 <- ggplot(freq[freq$ind < 5,], aes(x = reorder(rank, ind), y = Freq)) + 
...
Fig <- cowplot::plot_grid(plot_grid(p1, p2, ncol = 1, labels = c('A', 'B')),
                          plot_grid(p3.1, p3.2, p4, ncol = 1, labels = c('C', 'D', 'E')),
                          nrow = 1)
fig_count <- fig_count + 1
Figures[[fig_count]] <- Fig
...
names(Figures)[fig_count] <- fig.name
ggsave(filename = file.path(FigureFolder, fig.name), 
       plot = Fig, units = 'in', width = 8, height = 8)
...
# quantify prediction accuracy for finding complete/partial responders
m <- M[,top_vars]
f <- colData$binaryResponse
m <- m[!is.na(f),]
f <- f[!is.na(f)]

# heatmap of top vars 
p1 <- pheatmap::pheatmap(get_basis_matrix(m[,top_vars], f), silent = T)

# survival stratification using the top LF 
require(survminer)
require(survival)
top_lf <- dt[order(padj)][ref_cl == 'CR/PR'][1,]$variable
cp <- find_survival_cutpoint(m[,top_lf,drop = F], colData, event_col = 'censOS', duration_col = 'os')

p2 <- plot_survival(surv = colData, partition.df = data.frame('group' = ifelse(m[,top_lf] > cp, 'high', 'low'), 
                                                        row.names = rownames(m)), 
...
top_terms <- compute_geneset_enrichment(feats$rn, pathways)[order(pval)][padj < 0.05][1:10]
top_terms$term_wr <- stringr::str_wrap(gsub("_", " ", top_terms$term), width = 32)
p3 <- ggplot(top_terms, aes(x = reorder(term_wr, -pval), y = -log10(padj))) + 
  geom_bar(stat = 'identity') + 
  coord_flip() + labs(x = '') 
colnames(feats)[2] <- 'top_lf'
p4 <- ggplot(feats[1:10,], aes(x = reorder(rn, abs(top_lf)), y = abs(top_lf))) + 
...
cd <- colData #colData[!is.na(`Neoantigen burden per MB`)][!is.na(binaryResponse)]
m2 <- M[cd$bcr_patient_barcode,]
require(ggpubr)
df <- data.frame('response' = cd$binaryResponse, 
                 'TMB' = cd$`Neoantigen burden per MB`, 
                 'LF' = m2[,top_lf])
p5 <- ggpubr::ggscatter(df, x = 'LF', y = 'TMB', 
                        add = 'reg.line', cor.coef = T, cor.coef.coord = c(30, 10), 
                  cor.coef.size = 4) + labs(x = top_lf, y = 'Tumor Mutation Burden')

# immune phenotypes 
ip <- colData$`Immune phenotype`
#ip[ip == 'excluded'] <- NA
m3 <- M[!is.na(ip),]
ip <- ip[!is.na(ip)]

dt3 <- get_factor_specific_variables(m3, ip)[order(pval)][padj < 0.01]
ip_vars <- unique(dt3$variable)
ip_vars <- get_top_by_label(dt3, 'ref_cl', topN = 5)
res_ip <- predict_cluster_labels_glmnet(m3[,unique(dt3$variable)], ip, returnROC = T)
p6 <- plot_roc_list(res_ip)

pheatmap::pheatmap(get_basis_matrix(m3[,unique(ip_vars$variable)], ip), silent = T)

Fig <- cowplot::plot_grid(cowplot::plot_grid(p1, p2$gtable, p4, p3$plot, 
                                             labels = 'AUTO'), 
                          cowplot::plot_grid(p5, p6, p7, nrow = 1, 
                                             labels = c('E', 'F', 'G')), 
